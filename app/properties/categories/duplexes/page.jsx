  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        // Define qualifying property types for duplexes
        const duplexTypes = [
        ...
        // Filter properties to include only those matching duplex types
        const duplexProperties = allProperties.filter((property) =>
          duplexTypes.includes(property.propertyType)
        );
        ...
        if (userId) {
          ...
          const updatedProperties = duplexProperties.map((property) => {
    ...
    <>
      <section className="bg-primary-500 py-4">
        <div className="max-w-7xl mx-auto px-4 flex flex-col items-start sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold text-white mb-4">Duplexes</h2>
          <PropertySearchForm />
        </div>
      </section>
...
export default PropertiesDuplexes;
