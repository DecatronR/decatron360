  ...
  useEffect(() => {
    if (mapContainerRef.current && propertyLatitude && propertyLongitude) {
      const map = new mapboxgl.Map({
        ...
        center: [propertyLongitude, propertyLatitude], // Ensure the correct order
        zoom: 15,
      });

      // Add marker for the property
      const propertyMarker = new mapboxgl.Marker()
        .setLngLat([propertyLongitude, propertyLatitude]) // Ensure the correct order
      ...
      // Add buyer marker if location is available
      if (buyerLocation) {
